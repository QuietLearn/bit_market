 ...
 * 检查是否选中
 */
Kline.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        Kline.seItem = selected[0];
        return true;
    }
};
...
Kline.openAddKline = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/kline/kline_add'
    });
 ...
 */
Kline.openKlineDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/kline/kline_update/' + Kline.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除单个symbol批量成交记录
 */
Kline.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/kline/delete", function (data) {
            Feng.success("删除成功!");
            Kline.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("klineId",this.seItem.id);
        ajax.start();
    }
};
 ...
 * 查询单个symbol批量成交记录列表
 */
Kline.search = function () {
    var queryData = {};
    queryData['condition'] = $("#condition").val();
    Kline.table.refresh({query: queryData});
};

$(function () {
    var defaultColunms = Kline.initColumn();
    var table = new BSTable(Kline.id, "/kline/list", defaultColunms);
    table.setPaginationType("client");
    Kline.table = table.init();
});
